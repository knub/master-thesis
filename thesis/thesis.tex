...
Batmanghelich et al.~\cite{Batmanghelich2016} introduce a  introduce nonparametric spherical topic modeling.
...
% No-stemming (does harm in both \emph{word2vec} and topic modeling), as it destroys the difference between Apple and apples and (TODO: other example) as well as small/smallest.
...
In this chapter we start with a series of experiments with word similarity calculations in both topic models and embedding models, that motivate our newly-developed approach.
We will then present our approach, which is based on re-sampling words using information from the word embedding models.
We call our new model WELDA for \textbf{W}ord \textbf{E}mbedding \textbf{LDA}).
...
We ran initial experiments on the Wikipedia corpus\footnote{Full English Wikipedia dump from June 21st, 2016}, on which we trained both a topic model and a skip-gram word embedding model with two hundred dimensions.
We then evaluated the similarities between the words.
For embedding models, we take the cosine similarity between the two vectors.
For topic models, we build the topic distribution of a word by retrieving the word's probability in all topics in the topic-word matrix.
We then normalize these probabilities to one and treat it as a distribution.
To calculate the similarity between two words, we calculate the Jensen--Shannon divergence, which is a symmetrized and smoothed version of the Kullback--Leibler divergence:
% TODO: cite
\begin{equation*}
...
Even though both methods exploit the distributional hypothesis~\cite{Firth1957} the correlation of word similarities is surprisingly low.
...
\begin{table*}
       ...
       For example, the word pair (war, commander) in the cell (high topic model similarity, low word embedding similarity) means, that this pair is assigned a low similarity in the embedding space, but high similarity in the topic model space.
       ...
       \textbf{}                                                                                  & \textbf{} & \multicolumn{2}{l}{\textbf{Word embedding similarity}} \\
                                                                                                  &           & High                      & Low                       \\ \hline
         \multirow{2}{*}{\textbf{\begin{tabular}[c]{@{}l@{}}Topic model\\ similarity\end{tabular}}} & High    &
           (disease, diseases) \newline
           (professor, lecturer)
           &
           (africa, uganda) \newline
...
Our goal is to strengthen the topic themes by augmenting the top topic words with similar words from the word embedding space, but at the same not disregarding the bag-of-words document information completely.
...
The words have a high similarity to the existing words top words in the topic.
...
Before running running the Gibbs sampling, we load a word embedding model.
For all words in the corpus, we then retrieve its high-dimensional representation in the embedding space.
Afterwards, we use prinicipal component analysis to project the word vectors to a lower number of dimensions.
Finally, using the lower-dimensional new word vectors, we initialize a datastructure, that allows us to quickly find the nearest word given a vector in the embedding space.
...
Only then, we will run some additional Gibbs sampling iterations using our modified algorithm.
We run LDA first to obtain stable topic assignments and to start with relatively good topics as a baseline.
This is important, because before each Gibbs sampling iteration we estimate the topic distributions for all topics.
We use this topic distributions later in the innermost loop of the sampling algorithm.
If a coin flip with success probability $\lambda$ succeeds, we sample a new word from the word embedding space, based on the topic distribution of the new topic of the current word.
We use the previously initialized LSH or k-d tree data structure to find the nearest word near our drawn sample.
We then update the counts in the document-topic matrix and in the topic-word matrix, as if we had observed the sampled word, and not the word we observed in the text.
      ...
      \For{$d \in \{\textit{ 1 .. $|D|$ }\}$}
        \For{$w\_idx \in \{\textit{ 1 .. $N_d$ }\}$}
          ...
          \If{$coin\_flip(\lambda)\textit{ is successful}$}
            ...
            \State{$word\_id = nn\_search.find\_nearest\_word(sample)$}
          \EndIf
...
For that, we use the top words in all topics and retrieve their respective word vectors.
We then estimate a multivariate continuous distribution in the embedding space.
...
\begin{itemize}
  \item \textbf{Choice of topic distribution}:
    We test three distributions for the topic distributions.
...
When using a Gaussian distribution, we need to estimate two parameters: the mean vector $\mu \in R^n$ and the covariance matrix $Q \in R^{n \times n}$.
...
\begin{equation*}
  Q = \frac{1}{n} \sum\limits_{i=1}^n (x_i - \bar{x}) (x_i - \bar{x})^T
\end{equation*}
...
\begin{align*}
  \kappa_{n + 1} &= \kappa_n - \frac{A_p(\hat{\kappa_n})-\bar{R}}{1-A_p(\hat{\kappa_n})^2-\frac{p-1}{\hat{\kappa_n}}A_p(\hat{\kappa_n})},~where \\
...
We test different number of mixtures from one up to four and choose the one with the lowest Bayesian information criterion by Schwarz~\cite{Schwarz1978}.
Steele et al.~\cite{Steel2009} showed, that the BIC clearly outperforms other methods for choosing the number of mixtures in a Gaussian mixture model.
...
The number of free parameters $k$ for a mixture model is:
\begin{equation*}
...
After we obtained a sample from the topic distribution, we want to find the closest word to our sample in the embedding space.
The naive algorithm uses linear search and evaluates all words in the embedding space.
This is very expensive, especially because it happens in the innermost loop of Gibbs sampling and is therefore executed very often.
...
We pick the best method automatically in the $\textsc{Init}$ method.
\begin{figure}
       \centering
...
On the other side, Mikolov~\ref{Mikolov2013d} recommends almost no preprocessing except sentence detection.
Especially stop words are not removed in the word2vec approach:
Due to the window-based approach, where a word predicts its context words at the exact location, this would decrease performance.

This different preprocessing leads to different vocabularies and matching those is an important part of preprocessing for WELDA.
As the lowercasing step for topic models loses information, the matching is only approximate.
We display the matching in Algorithm~\ref{alg:vocabulary_matching}.
...
With this approach, we force the first topic to appear in almost all documents.
This forces LDA to put words in this topic, which occur in many documents and are very generic.
% TODO: Reference section where we talk about nltk stopword list

We experimented with different values of $\alpha_0^{boost}$ and show the results in Table~\ref{table:alpha0_boost}.
The second row shows the percentage of documents containing the first topic and the third row shows the percentage of words in the first topic.
We see, that a value of fifteen is sufficient, so that the topic occurs in more than ninenty percent of the documents.
\begin{table}[]
  \centering
  \caption{Testing by which factor $\alpha_0$ must be multiplied, so that the first topic occurs in many documents.
  All values in the last two rows are percentages.}
  \label{table:alpha0_boost}
  \begin{tabular}{c||cccccccccc}
  $\mathbf{\alpha_0^{boost}}$                & 1     & 2     & 3     & 5     & 7     & 9     & 10    & 15    & 20    & 30    \\
    \hline
  \textbf{doc. with first topic}  & 11  & 23  & 39  & 47  & 56  & 65  & 79  & 90  & 91  & 91  \\
  \textbf{percentage first topic} & 2   & 4   & 8   & 10  & 13  & 16  & 19  & 24  & 24  & 24
  \end{tabular}
\end{table}
Indeed, if we look at the top words in the first topic in the \textsc{20News} corpus, we find corpus-specific stop words: \\
\noindent\fbox{
    \parbox{0.8\textwidth}{
...
We then always resample, when one of these words occurs, even if the coin toss in the WELDA algorithm failed.
Due to this, we differentiate between $\lambda$ and $\lambda_{act}$ in experiments.
...
This procedure helps replace generic words, while at the same time removing overly general words.
In theory, WELDA should have this property automatically, because often occurring words are more likely to be replaced.
Also, this words are usually far away from the means of the topic distributions.
Thus, these words disappear from the topics.
Replacing stop words should speed up this process.
...
Our WELDA implementation is written in Scala and available on GitHub\footnote{https://github.com/knub/master-thesis}.
The initial LDA is run with Mallet~\cite{McCallum2002}.
The word vectors are trained using gensim~\cite{Rehurek2010}.
For loading the word vectors for WELDA, we use the Deeplearning4j\footnote{https://deeplearning4j.org/} library.
...
We use two different corpora for topic modeling, the \textsc{20News} corpus and the \textsc{Nips} corpus.
...
Words were not lowercased and no stop words were removed, as both have been show to harm the performance of the resulting vectors.
% TODO: Quote
