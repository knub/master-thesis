...
We will now explain the Gibbs sampling algorithm in detail, see Algorithm~\ref{alg:gibbs_sampling}.
The algorithm starts by assigning topics randomly to words (line 2).
Each word gets exactly one topic.
Then, for a number of iterations $M$ (line 3), the algorithm iterates over all documents and all words in the documents (lines 4-7).
The main idea is now to repeatedly sample new topics for all words in the documents.
During sampling, all other topic assignments are assumed fixed and the conditional distribution over the topics for the current word are considered (line 10).
The sampling occurs from the multinomial distribution defined by:
\begin{equation*}
  p(z_w = t | Z) \propto (N_{d}^{t} + \alpha_t) \times  \frac{N^{t, w} + \beta}{N^{t} + V * \beta},
\end{equation*}
where $N_{d}^{t}$ is the number of times the topic $t$ occurred in the current topic, $N^{t, w}$ is the number of times the current word $w$ occurred with topic $t$ and $N^t$ is the number of times topic $t$ occurred in all documents.
Note that all these numbers are counted without the current word taken into account (lines 8 and 11).
...
It is guaranteed, that this repeated sampling approximates the joint distribution of all variables.
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Procedure{Sampling}{Documents D}
    ...
    \For{$n \in \{\textit{ 1 .. M }\}$}
      \For{$d\_idx \in \{\textit{ 1 .. $|D|$ }\}$}
        ...
        \For{$w\_idx \in \{\textit{ 1 .. $|doc|$ }\}$}
          \State{$word\_id \gets doc[w\_idx]$}
...
% TODO: We aim to combine the two methods to achieve better topic models (Tanja: concrete goal is missing, not clear so far)
...
We then evaluated the similarities between the words. % TODO: Tanja: Begründung fehlt, "between the words to achieve xyz".
...
To calculate the similarity between two words, we calculated the Jensen--Shannon divergence, which is a symmetrized and smoothed version of the Kullback--Leibler divergence:
% TODO: cite
\begin{equation*}
...
% TODO Tanja: An wichtigen Stellen nochmal referenzieren
...
% TODO Tanja: Link with "Algorithm X, Line Y
\begin{itemize}
...
This is very expensive, especially because it happens in the innermost loop of Gibbs sampling and is therefore executed very often, approximately $\lambda * N * N_{words}$ times.
% TODO: Explain N_words
...
% TODO: first topic === background topic
...
We see, that a value of fifteen is sufficient, so that the topic occurs in more than ninety percent of the documents.
\begin{table}[]
  \centering
  ...
  $\mathbf{\alpha_0^{boost}}$                & 1     & 2     & 3     & 5     & 7     & 9     & 10    & 15    & 20    & 30    \\
    \hline
  \textbf{doc. containing first topic}  & 11  & 23  & 39  & 47  & 56  & 65  & 79  & 90  & 91  & 91  \\
  \textbf{words in first topic} & 2   & 4   & 8   & 10  & 13  & 16  & 19  & 24  & 24  & 24
  \end{tabular}
\end{table}
...
% TODO: Nächster Satz macht keinen Sinn
This procedure helps replace generic words, while at the same time removing overly general words.
% TODO: should have
% TODO: Ganzer Absatz noch nicht klar, hypothesis klarer definieren
