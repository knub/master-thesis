...
For example, an author could decide to write a document about \emph{politics} (70~\%) in \emph{sports} (30~\%) and then pick the words ``coalition'', ``election'' and ``corruption'' for \emph{politics} and the words ``soccer'' and ``ball'' for \emph{sports}.
...
Especially the work of Mikolov et al.~\cite{Mikolov2013,Mikolov2013a,Mikolov2013b} and the publication of the open-source tool \emph{word2vec}, which allows training on billions of words in a realistic time frame, must be mentioned.
...
\clearpage
% \phantom{This sentence is here to fill some phantom space.}
...
\begin{figure}
       \centering
       \includegraphics[width=\textwidth]{figures/lda_showcase.png}
...
However, this is an extensive and time-consuming approach in itself and needs to be done again for every new topic model with its own new topics.
...
Altogether, Röder et al.~\cite{Roder2015} evaluated a total of $237,912$ coherence measures, which they created by parameterizing each step of the computation and testing the cross product of all parameter settings.
...
Interestingly, this relationship holds for both semantic and syntactic similarity~\cite{Mikolov2013a}.
The word vector closest to the result of
\begin{equation*}
...
However, this representation is not good at keeping linear relationships~\cite{Mikolov2013a,Mikolov2013b}.
...
We will then present our approach, which is based on resampling words using information from the word embedding models.
...
We will present the modified Gibbs sampling algorithm and discuss a series of important design decisions.
...
We normalized these probabilities to one and treat it as a distribution.
...
Zero corresponds to the highest similarity because in this case the probability distributions are identical.
...
\begin{algorithm}[H]
  \caption{Estimating distribution parameters for each topic. This algorithm is run before each Gibbs sampling iteration.}
...
\begin{itemize}
  ...
  \item \textbf{Resampling probability $\boldsymbol{\lambda}$} (Algorithm~\ref{alg:welda_gibbs_sampling}, line 10):
    The parameter $\lambda$ decides, how often a word is resampled from the topic distributions.
    Thus, the parameter $\lambda$ governs, how much extra information from the word embeddings is leaked into the topic model.
    ...
    We will later determine, which value of $\lambda$ constitutes a good compromise between the two models.
  ...
  \item \textbf{Number of dimensions for dimensionality reduction $\boldsymbol{N_{pca}}$} (Algorithm~\ref{alg:welda_init}, line 5):
    ...
    Fewer words are better because in this case we can use the really good top words at the start of a topic, and not the later ones.
    ...
    In general, we expect a higher dimensionality to perform better.
  \clearpage
...
Note the logarithmic y-axis, so the ratio actually increases almost exponentially.
...
We assume that WELDA will not work in this case.
\clearpage
...
We removed stop words, using the stop words list from the Python nltk~\cite{Bird2009} toolkit.
...
We only considered article pages and furthermore discarded disambiguation pages, lists and tables.
...
\begin{table}[]
  ...
  For comparison, we also added the Google News embeddings, which were published by Google.}
...
In this section, we first describe our evaluation metric $C_V$ in detail.
We then evaluate WELDA's design decisions with respect to topic coherence in several experiments.
...
In general, the higher the value, the higher the topic coherence, which confirms our assumption from Section~\ref{sec:welda_algorithm}.
...
Altogether, we can conclude that von Mises-Fisher distributions perform slightly worse overall than the Gaussian distributions, especially with a higher dimensionality of the word embeddings.
The theoretically better representation of the models did not manifest in better experimental results.
Gaussian distributions and Gaussian mixtures show the same performance.
There is no additional benefit from using the mixture model, so the Gaussian model is preferable.
...
Therefore, the resampling sometimes samples words, which do not occur in the document at all.
...
The plot for the $N_{pca}$ experiment is displayed in Appendix~\ref{sec:appendix_welda_dc_ntop}.

\chapter{Comparative evaluation of similar approaches}
...
Almost the same top words occurred in all topics.
...
\begin{table}[]
  ...
  \begin{tabular}{lrrrcrrr}
    ...
    \multirow{2}{*}{} & \multicolumn{3}{c}{$K = 50$} & \phantom{a} & \multicolumn{3}{c}{$K = 250$} \\ \cmidrule{2-4} \cmidrule{6-8}
                      ...
                      WELDA             & 58.1    & 58.3    & 57.4   && 66.8    & 64.1     & 64.5   \\
...
Our Experiments were conducted on a 32-core, Intel Xeon CPU E7-8837 @ 2.67GHz machine with 256 GB main memory.
...
We tested on \textsc{20News} with 200-dimensional embeddings and on \textsc{Nips} with 50-dimensional corpus-specific embeddings.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lp{4cm}rr@{}}
    ...
    WELDA Gaussian, $\lambda = 0.5$ & ~~~~~~~~~~~~~~~~\xmark                     &   97 min                            &  109 min \\  % 4027 sec,
...
\begin{figure}[H]
       \centering
       \includegraphics[width=12cm]{figures/welda_lambda_runtime.png}
       \caption{Runtime of WELDA on the \textsc{20News} corpus depending on the resampling parameter $\lambda$ with Wikipedia embeddings and $N_{pca} = 10$.}
...
These topic distributions capture and generalize the positions of the words in the embedding space.
By learning an entire distribution and not only a single topic vector, we are also capturing the covariance of the word positions in the embedding space.
...
Our experiments with different topic distributions indicated best performance with Gaussian distributions as topic distributions.
...
If LDA were used to learn different word embeddings for different meanings, then the improved word embeddings could in turn help WELDA create better topics.
...
% one might be fine with only one topic about \emph{sports}, but wants several fine-grained topics about \emph{politics}.
...
\chapter{Plots}
  ...
  \begin{figure}[H]
         ...
         \caption{Von Mises-Fisher WELDA's topic coherence development on \textsc{20News} for different $\lambda_{act}$ values with corpus-specific embeddings. At the end, we plot the respective $TC$ values for Gaussian WELDA (diamonds), which is superior in this case.}
  ...
  \newpage
  \subsection{Document classification performance depending on $N_{pca}$}
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{lrrrcrrr}
      ...
      \multirow{2}{*}{} & \multicolumn{3}{c}{$K = 50$} & \phantom{a} & \multicolumn{3}{c}{$K = 250$} \\ \cmidrule{2-4} \cmidrule{6-8}
                        & $Prec$    & $Rec$     & $F1$     && $Prec$    & $Rec$   & $F1$     \\ \midrule
                        Only AE                          & 78.6    & 78.1    & 78.0   && \multicolumn{3}{c}{--------------------}        \\
                        LDA                              & 71.1    & 71.2    & 70.1   && 73.2    & 72.8     & 72.6   \\
