...
\begin{table}[]
  ...
  $\mathbf{\alpha_0^{boost}}$                & 1     & 2     & 3     & 5     & 7     & 9     & 10    & 15    & 20    & 30    \\
    \hline
  \textbf{doc. containing background topic}  & 11  & 23  & 39  & 47  & 56  & 65  & 79  & 90  & 91  & 91  \\
  \textbf{words in background topic} & 2   & 4   & 8   & 10  & 13  & 16  & 19  & 24  & 24  & 24
  \end{tabular}
\end{table}
...
Before we start with these results, we will explain, which word embeddings we used, as this is an important decision for the algorithms.
...
We only considered article pages and then furthermore discarded disambiguation, lists and tables.
...
Not surprisingly, the embeddings trained on the topic model corpora .
This can also be explained by the fact, the both \textsc{20News} and \textsc{Nips} have a special focus.
...
\begin{table}[]
  ...
  We also added the Google News embeddings, which were published by Google.}
  \label{table:word_embeddings_performance}
  \begin{tabular}{ccccc}
    \toprule
    \textbf{Data set} & \textbf{Dimensions} & \textbf{\# words} & \textbf{\# sentences} & \textbf{Accuracy AR} \\
    \midrule
    \textsc{English Wikipedia} & 200                 & 1,787,888,109     & 93,322,949            & 56.5 \%              \\
    \textsc{English Wikipedia} & 50                 & 1,787,888,109     & 93,322,949            & 40.4 \%              \\
    \textsc{20News}   & 50                  & 5,705,876         & 293,467               & 11.1 \%              \\
    \textsc{Nips}     & 50                  & 4,973,427         & 212,400               & 10.1 \%              \\
    \textsc{Google News} & 300                 & 100 billion       & \textit{unknown}      & 74,3 \%              \\
    \bottomrule
  \end{tabular}
\end{table}
...
We again use the \textsc{20News} corpus.
The corpus was collected from twenty different newsgroups, which are shown in Table~\ref{table:20news}.
All together, the corpus contains 11293 documents.
...
We ran experiments for all approaches using $T = 50$ and $T = 250$.

For classification, we ran a three-fold stratified cross validation.
In each fold, we used a linear \emph{l}-1 regularized SVM classifier, where we trained $T$ one-vs-rest classifiers.
...
\begin{itemize}
  ...
  \item
    ...
    In their paper, they learn 14 topics separately on each classification category and then combine the, resulting in 280 topics.
...
Note: Gaussian LDA had to be trained with 50-dimensional word embeddings trained on Wikipedia.

As with topic coherence, 
...
\begin{table}[]
  ...
  \begin{tabular}{lrrrcrrr}
    ...
    \multirow{2}{*}{} & \multicolumn{3}{c}{$K = 50$} & \phantom{a} & \multicolumn{3}{c}{$K = 250$} \\ \cmidrule{2-4} \cmidrule{6-8}
                      & $Prec$    & $Rec$     & $F1$     && $Prec$    & $Recall$   & $F1$     \\ \midrule
                      LDA               & 0.xx    & 0.xx    & 0.xx   && 0.xx    & 0.xx     & 0.xx   \\
                      Average WE        & 68.2    & 67.9    & 67.0   && \multicolumn{3}{l}{}        \\
                      LFTM              & 0.69    & 0.67    & 0.66   && 0.71    & 0.71     & 0.71   \\
                      TopicVec          & 0.35    & 0.34    & 0.31   && 0.41    & 0.37     & 0.34   \\
                      Gaussian LDA      & 0.42    & 0.43    & 0.40   && 0.xx    & 0.xx     & 0.xx   \\
                      WELDA             & 0.62    & 0.57    & 0.57   && 0.68    & 0.66     & 0.66   \\
  \bottomrule
  \end{tabular}
\end{table}
...
As a summary, most methods are able to generate better topics,
at the expense of loosing accuracy in a document classification task.
