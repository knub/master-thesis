...
% \usepackage[left=0.82675in,textwidth=5.78853in]{geometry}
...
\usepackage{layouts} % printinunitsof % TODO
\usepackage{amsmath}
\usepackage{amssymb}
...
%%% Option 1:
% \usepackage[backend=bibtex, bibstyle=numeric, citestyle=numeric, sorting=nty, maxnames=9, doi=false, isbn=false]{biblatex}
%%% Option 2:
...
\printinunitsof{in}\prntlen{\hoffset}
\printinunitsof{in}\prntlen{\oddsidemargin}
\printinunitsof{in}\prntlen{\evensidemargin}
...
% In Appendix~\ref{sec:appendix_welda_vmf}, we also show the plots for the topic coherence development through the iterations. TODO
...
We shift probability mass from general, low-information words to specific, high-information words, which strenghten the main theme of the topic.
...
Our experiments with different topic distributions indicated
Finally, We tested different Gaussian distributions, von Mises-Fisher distributions and Gaussian mixtures to 
We especially showed that corpus-specific embeddings, albeit with a , can still help to improve topic coherence.
...
The word similarity for polysemous words needs to c
As LDA 


An idea would be to 
Topic specific replacement
Our topic distributions are currently estimated from the data.
By setting priors on the topic distributions one could define a certain set of expected topics.
For example, when learning topics on the \textsc{20News} corpus, one might be fine with only one topic about \emph{sports}, but want several topics fine-grained topics about \emph{computer science}.
