...
Viele Modelle für den natürlichsprachigen Text wurden von dieser Annahme motiviert.
Zwei dieser Methoden sind Topic-Modelle und Vektorrepräsentationen von Wörtern.
...
Wir erreichen dies durch die Schätzung von Themen-Verteilungen im Wort-Vektorraum und der Stichprobe neuer Wörter aus dieser Verteilung, um die Themen zu stärken.
...
When building new topic models, which use word embeddings, our goal is to incorporate a priori semantic information about words into the topic model to increase the coherence of the topics.
...
Stop words were not relevant for the average vector calculation, as they were already removed during the topic model preprocessing.
...
\label{sec:tc_contenders}

We now present the different parameter settings for the contenders.
...
\begin{itemize}
  ...
  \item
    \textbf{NSTM:}
    As mentioned in Section~\ref{sec:tc_contenders} we could not obtain any results for the NSTM method on the \textsc{20News} corpus, so we had to exclude it from the analysis.
\end{itemize}
