...
Topics models have also been applied in bioinformatics, where they help understand evolution~\ref{Pritchard2000}.
...
Although the idea is quite old~\cite{Rumelhart1988,Bengio2003}, the model has recently gained a lot of popularity in the research community.
This is because of the performance improvements of CPUs and GPUs and because of better training algorithms.
Especially the work of Mikolov et al.~\cite{Mikolov2013,Mikolov2013a,Mikolov2013b} and the publication of the open-source tool \emph{word2vec}, which allows to train on billions of words in a realistic time frame, must be mentioned.
The training in \emph{word2vec} works in a supervised manner.
A sliding window is run over the text, and each word must try to predict its surrounding words.
...
\begin{itemize}
  ...
  \item
    Recently, there has been a lot of research on combining these topic models and word embeddings~\cite{Li2016,Nguyen2015,Das2015,Batmanghelich2016}.
...
Together, this would form a large matrix with the rows being the documents and the columns being the words.
...
\paragraph{Latent Feature Topic Models (LFTM)}
Another approach, called Latent Feature Topic Models, is taken by Nguyen et al.~\cite{Nguyen2015}.
...
\paragraph{Nonparametric Spherical Topic Model (NSTM)}
...
\paragraph{Generative Topic Embedding (TopicVec)}
...
For the topic models, this is only noise and we do not want this noise in the topics.
...
\begin{enumerate}
  ...
  \item \textbf{Confirmation Measure}:
    ...
    \begin{equation*}
      ...
      \end{pmatrix}
    \end{equation*}
    The dimension of the vector $\vec{v}$ is always equal to the number $N$ of top words.
...
\begin{figure}
       ...
       \caption{Average topic coherence for a fixed $\alpha$ respectively $\beta$, while averaging over all values of $\beta$ respectively $\alpha$. There is no trend recognizable beyond random fluctuations. Therefore, $\alpha$ and $\beta$ have no influence on the topic coherence.}
...
\begin{figure}
       ...
       \caption{Development of the topic coherence for LDA on the \textsc{20News} corpus. The highest improvements occur in the first 200 iterations. After iteration 800, the topic coherence is constant beyond random fluctuations.}
...
The plots in the following sections always use the actual $\lambda_{act}$, i.e. how often was a word replaced during sampling, either because it was a stop word or because the coin toss was successful.
...
For $\lambda = 0.0$ the topic coherence does not change afterwards beyond random fluctuations.
...
\subsubsection{Influence of parameters $N_{pca}$ and $N_{top}$}
...
We conducted experiments with $\lambda = 0.5$ and the Wikipedia embeddings on the \textsc{20News} corpus.
...
However, large differences are visible for $N_{pca}$.
...
% TODO: PCA dimensions vs topic similarity evaluation?
...
\begin{figure}
       ...
       \caption{Topic coherence for different values of $N_{pca}$ and $N_{top}$. We also plotted a regression line indicating the development for increasing $N_{top}$. $N_{top}$ does not have a huge influence on the topic coherence. In contrast, the higher $N_{pca}$ the better the topic coherence.}
...
When we replace these words, the $\lambda$ is not equal to the actual $\lambda_{act}$, as we replace more words in that case.
We tested with $\lambda_{act} = 0.2$ and $\lambda_{act} = 0.5$.
To ensure similar values when using background word sampling and to not influence the results by different $\lambda$ values, we tested different $\lambda$ parameters.
After same experiments, we found $\lambda = 0.10$ and $\lambda = 0.45$ to result in $\lambda_{act} = 0.2$ and $\lambda_{act} = 0.5$.
...
We run standard LDA before WELDA to estimate the topic distributions on already good topics.
...
% \subsubsection{Choice of word embeddings} TODO
...
% \subsubsection{Top word selection} TODO

\subsubsection{WELDA with von Mises-Fisher distributions}

So far, all experiments were executed with Gaussian distributions as the topic distributions.
We now evaluate WELDA with von Mises-Fisher distributions.
...
\begin{table}[]
  \centering
  \caption{Comparison of WELDA's topic coherence with Gaussian distributions and with von Mises-Fisher (vMF) distributions on \textsc{20News}. The $\Delta TC$ columns indicate the difference. A negative value indicates that von Mises-Fisher distributions are worse.}
  \label{table:welda_vmf}
  \begin{tabular}{@{}ccccccc@{}}
    \toprule
           &           & \multicolumn{2}{c}{\textsc{20News}} && \multicolumn{2}{c}{Wikipedia} \\ \cmidrule{3-4} \cmidrule{6-7}

           $\lambda$ & $\lambda_{act}$ & $TC~vMF$             & $\Delta TC$&           & $TC~vMF$         & $\Delta TC$      \\
           \midrule
           0.2    & 0.29      & $45.8$         & $\pm 0.0$         && 45.7       & $+0.7$           \\
           0.4    & 0.47      & 45.3           & $-1.8$            && 45.9       & $+0.3$           \\
           0.5    & 0.56      & 45.6           & $-1.1$            && 46.0       & $+0.5$           \\
           0.6    & 0.64      & 45.1           & $-0.4$            && 45.4       & $+0.6$           \\
           0.8    & 0.82      & 45.0           & $-0.9$            && 44.8       & $+1.4$           \\ \bottomrule
  \end{tabular}
\end{table}

\subsubsection{WELDA with Gaussian mixtures}
...
\begin{itemize}
  ...
  \item
    ...
    When training with higher-dimensional embeddings, Gaussian LDA does not converge and yield interpretable results.
  ...
  \item
    ...
    In contrast to the other methods, TopicVec needs different preprocessing, as it is trained on the context of a word, similarily to the skip-gram architecture.
...
\begin{table}[]
  \ra{1.3}
  \centering
  \caption{Topic coherence on the \textsc{20News} and \textsc{Nips} corpus}
  \label{table:topic_coherence}
  \begin{tabular}{@{}lrrcrr@{}}
    \toprule
                 & \multicolumn{2}{c}{\textsc{20News}} && \multicolumn{2}{c}{\textsc{Nips}} \\ \cmidrule{2-3} \cmidrule{5-6}
                              & Wikipedia     & \textsc{20News}     && Wikipedia     & \textsc{Nips}     \\
                 \midrule
                              LDA          & \multicolumn{2}{c}{41.9}   && \multicolumn{2}{c}{40.6} \\
                              Gaussian LDA & 38.5          &            && 39.2          & TODO     \\
                              LFTM         &               & 45.6       &&               &          \\
                              NSTM         & --            & --         && 37.5          &          \\
...
\begin{table}[]
  ...
  \begin{tabular}{@{}lp{4cm}cc@{}}
    \toprule
                 ...
                 LFTM, $\lambda = 0.6$  & ~~~~~~~~~~~~~~~~\xmark                                 & 61.2 h          & 71 min \\
                 TopicVec               & ~~~~~~~~~~~~~~~~\cmark                                 & 178 min         & \\
                 Gaussian LDA           & ~~~~~~~~~~~~~~~~\xmark                                 & 14.1 h          & \\ % 50713 seconds
                 WELDA, $\lambda = 0.5$ & ~~~~~~~~~~~~~~~~\xmark                                 & 67 min          & \\ \bottomrule % 4027 sec,
  \end{tabular}
\end{table}
...
\begin{appendices}
  ...
  \subsection{Document classification on 20News with corpus-specific embeddings}
  ...
  \begin{table}[]
    ...
    \begin{tabular}{lrrrcrrr}
      ...
      \multirow{2}{*}{} & \multicolumn{3}{c}{$K = 50$} & \phantom{a} & \multicolumn{3}{c}{$K = 250$} \\ \cmidrule{2-4} \cmidrule{6-8}
                        ...
                        Gaussian LDA             & 53.7    & 54.3    & 52.3   && xxxx    & xxxx     & xxxx   \\
