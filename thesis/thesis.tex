...
% TODO: Add previous methods like held-out likelihood etc, but that did not correlate very well
A common evaluation metric for topic models is \emph{topic coherence}.
Topic coherence measures, how well the top-words in the topics belong together and form a common theme.
...
The goal of a topic coherence metric is to capture the this usefulness of a topic for a human user.
Therefore, the goal of topic coherence measure is to have a high correlation with human ratings.
The base idea for such a coherence measure was introduced by Newman et al.~\cite{Newman2010}.
They evaluated metrics based on WordNet, Google searches and word-cooccurrences in Wikipedia articles, and found that latter provides the best metric.
Röder et al.~\cite{Roder2015} build on this idea and evaluated a total of 237912 coherence measures, which they created by parameterizing each step of the computation and then testing the cross product of all parameter settings.

We will use this metric, which is called $C_V$ in the original paper, to evaluate topic coherence throughout our master thesis.

All

cite the care and feeding paper
...
Foo
\section{WELDA Experiments}

\subsection{Locality Sensitive Hashing for nearest neighbour search}
